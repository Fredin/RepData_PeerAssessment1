...
Interval no. `r max_interval` or, in other words, the interval from 8:35:00 to 8:39:59 AM was the most active with `r round(maxunit)` steps on average.
...
It's true. The subject is on average more active during the weekend. Let's do a [paired t-test](http://en.wikipedia.org/wiki/Student%27s_t-test#Paired_samples) to determine if this difference is statistically significant.

```{r Weekends vs. Weekdays T-test}
# We'll need to take a random sample to match the amount of observations, plus a little scrambling will make the result more robust. 
set.seed(1234)
weekday_sample<-sample_n(days,16)
weekend_sample<-sample_n(ends,16)
t.test(weekend_sample[,2], weekday_sample[,2], paired=TRUE, alternative = "g")
```

It seems the difference is very likely to be due to chance alone, too bad. There's more we could do with the data, like testing if morning behavior is significantly different between conditions, but let's end this exercise here.